...
public class ChunkManager {
    ...
    public void handleExplosion(EntityExplodeEvent event) {
        ...
        } else if ((Util.isLiquid(detonatorLoc.getBlock().getType())) && (ConfigManager.getInstance().getFluidsProtectIndustructables())) {
            ...
            for (int y = radius; y >= -radius; y--) {
                for (int z = -radius; z <= radius; z++) {
                    ...
                    if (ConfigManager.getInstance().getBypassAllFluidProtection()) {
                        ...
                        if (distance < radiuz - 0.1 && (Util.isNearLiquid(targetLoc) || Util.isLiquid(targetLoc.getBlock().getType()))) {
                            ...
                            if (MaterialManager.getInstance().contains(targetLoc.getBlock().getType().name())) {
                                ...
                                if (!Util.isLiquid(targetLoc.getBlock().getType()) && damper > 0 && damper >= Math.random()) {
        ...
        for (Block block : explosionEvent.totalBlockList()) {
            ...
            } else if (Util.isLiquid(block.getType()) && event.getEntity().hasMetadata("ObbyLiquidEntity")) {
